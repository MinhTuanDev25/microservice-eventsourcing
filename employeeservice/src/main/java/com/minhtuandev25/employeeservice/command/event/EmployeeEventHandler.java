...
public class EmployeeEventHandler {
	...
	public void on(EmployeeUpdatedEvent event) throws Exception {
        ...
        Employee employee = oldEmployee.orElseThrow(() -> new Exception("Employee not found for projection update: " + event.getId()));
	...
	public void on(EmployeeDeletedEvent event) {
        try {
            employeeRepository.findById(event.getId()).orElseThrow(() -> new Exception("Employee not found for projection delete: " + event.getId()));
            employeeRepository.deleteById(event.getId());
        } catch (Exception e) {
            log.error("Employee not found for projection delete: {}", event.getId());
        }
	}
}
